...
birds$devo_mode_full <- relevel(birds$devo_mode_full, ref = "precocial")
model_alloc_f_res_full <- MCMCglmm(res_brain ~ devo_mode_full + weight + time_fed + res_egg_mass + clutch_size + caretakers +
                                devo_mode_full:weight + devo_mode_full:time_fed +
                                res_egg_mass:devo_mode_full + 
                                res_egg_mass:weight +
                                clutch_size:devo_mode_full + clutch_size:weight + clutch_size:res_egg_mass,
                              random = ~ tip_label,
                              ginverse = list(tip_label = tree_jetz_data_AINV$Ainv),
                              prior = prior1,
                              data = birds,
...
model_combined_full <- MCMCglmm(brain_log ~ devo_mode + weight + time_fed + res_egg_mass + clutch_size + caretakers +
                                  devo_mode:weight + devo_mode:time_fed + res_egg_mass:devo_mode + 
                                  res_egg_mass:weight + 
                                  clutch_size:devo_mode + clutch_size:weight + clutch_size:res_egg_mass +
...
##### larger models with less predictors ----


## NO TIME-FRD
source(file = 'data_load_notimefed.R')
phylo_vcv <- vcv.phylo(birdtree.Jetz, corr = T)
...
model_no_timefed <- MCMCglmm(res_brain ~ devo_mode + weight +res_egg_mass + clutch_size + caretakers +
                               devo_mode:weight + 
                             ...
                             data = birds,
                             nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
summary(model_no_timefed)


## NO TIME-FED & NO grouping
source(file = 'data_load_notimefed_no_grouping.R')
phylo_vcv <- vcv.phylo(birdtree.Jetz, corr = T)
...
model_no_timefed_grouping <- MCMCglmm(res_brain ~ devo_mode + weight +res_egg_mass + clutch_size + caretakers +
                                        devo_mode:weight + 
                                        res_egg_mass:weight + 
                                        clutch_size:devo_mode + clutch_size:weight + clutch_size:res_egg_mass +
                                        res_egg_mass:devo_mode +
                                        ...
                                        foraging_3 + move_max + 
                                        food_energy_RC + food_h_level_nofibres_RC + social_bonds + insularity + colonial + 
                                        temp_range + temp_rain + temp_stdv +
                                        foraging_3:weight + move_max:weight + 
                                      ...
                                      data = birds,
                                      nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
summary(model_no_timefed_grouping)


#### bVIF analysis ----

birds <- gdata::drop.levels(birds)
levels(birds$grouping) <- c("as_pair", "lar_sma", "as_pair", "lar_sma")
...
# define the full formula removing interactions
final_formula <- res_brain ~ devo_mode + weight + time_fed + res_egg_mass + clutch_size + caretakers +
  foraging_3 + move_max + 
  food_energy_RC + food_h_level_nofibres_RC + social_bonds + insularity + colonial + grouping + 
  temp_range + temp_rain + temp_stdv
all.vars(final_formula)[-1] -> dep_vars
VERB <- F
eff.samp.threshold <- 750
...
for (i in 1:length(dep_vars)) {
  ...
  reg_dep <- variable
  reg_indep <- dep_vars[-which(dep_vars == variable)]
  
  vif_formula <- as.formula(paste(reg_dep, '~', paste(reg_indep, collapse = '+')))
  
  if(is.numeric(unlist(birds[which(names(birds) == reg_dep)]))) {
    vif_family <- 'gaussian'
    vif_prior <- list(R = list(V = 1, nu = 0.002),
                      G = list(G1 = list(V = 1, nu = 0.002, alpha.mu = 0, alpha.V = 1e5)))
    c <- 1
    CONST <- 0
  } else {
    vif_family <- 'categorical'
    vif_prior <- list(R = list(V = 1, fix = 1),
                      G = list(G1 = list(V = 1, nu = 0.002, alpha.mu = 0, alpha.V = 1e5)),
                      B = list(mu = rep(0, 17), V = diag(17)*(1+pi^2/3)))
    c <- 2
    CONST <- pi^2/3
  }
  
  vif_model <- MCMCglmm(fixed = vif_formula, random = ~ tip_label,
                        ginverse = list(tip_label = tree_jetz_data_AINV$Ainv),
                        nitt = c*30000, thin = c*22, burnin = c*8000, verbose = VERB,
                        data = birds,
                        prior = vif_prior,
                        family = vif_family)
  
  cat("MCMC model for "); cat(variable); cat(" done!\n")
  internal_list$status <- 0
  
  while(any(summary(vif_model)$solutions[,'eff.samp'] < eff.samp.threshold)) {
    N_autocorr <- sum(summary(vif_model)$solutions[,'eff.samp'] < eff.samp.threshold)
    cat("Repeating model due to autocorrelations. "); cat(N_autocorr); cat(" out of "); cat(length(reg_indep)+1); cat(" bad\n")
    c <- c * 2
    ...
    if(c*3e4 > 4000000) {
      cat(variable); cat(" may not be reliable due to autocorrelation issues, breaking at NITT "); cat(c*3e4); cat("\n")
      internal_list$status <- N_autocorr
      break
    }
  }
  
  internal_list$var_name <- reg_dep
  internal_list$model <- vif_model
  
  fs <- as.numeric(vif_model$X %*% summary(vif_model)$solutions[,1])
  R2 <- var(fs)/(var(fs) + mean(vif_model$VCV[,"tip_label"]) + mean(vif_model$VCV[,"units"]) + CONST)
  
  internal_list$R2 <- R2
  internal_list$VIF <- 1/(1-R2)
  
  output_list[[i]] <- internal_list
}

output_list %>%
  map_dfr(~ data.frame(.x$var_name, .x$VIF))

save(output_list, file = "211001_VIF_output.Rdata")


#### supporting models with brain as predictor - longevity as response ----
...
model_brain_longevity <- MCMCglmm(longevity_t ~ res_brain + devo_mode + weight +
                                   devo_mode:weight
                                 ,
                                 random = ~ tip_label,
                                 ginverse = list(tip_label = tree_jetz_data_AINV$Ainv),
                                 ...
                                 data = birds,
                                 nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
autocorr.plot(model_brain_longevity$Sol)
autocorr.plot(model_brain_longevity$VCV)
summary(model_brain_longevity)


#### supporting models with brain as predictor - egg mass as response ----
...
model_brain_eggmass <- MCMCglmm(egg_mass ~ res_brain + devo_mode + weight +
                                    devo_mode:weight
                                  ,
                                  random = ~ tip_label,
                                  ginverse = list(tip_label = tree_jetz_data_AINV$Ainv),
                                  ...
                                  data = birds,
                                  nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
autocorr.plot(model_brain_eggmass$Sol)
autocorr.plot(model_brain_eggmass$VCV)
summary(model_brain_eggmass)


#### supporting models with brain as predictor - clutch as response ----
...
model_brain_cs <- MCMCglmm(clutch_size ~ res_brain + devo_mode + weight +
                                  devo_mode:weight
                                ,
                                random = ~ tip_label,
                                ginverse = list(tip_label = tree_jetz_data_AINV$Ainv),
                                ...
                                data = birds,
                                nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
autocorr.plot(model_brain_cs$Sol)
autocorr.plot(model_brain_cs$VCV)
summary(model_brain_cs)

## additional plots

plot1 <- ggplot(data = birds, aes(x = res_brain, y = longevity, col = devo_mode)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()

plot2 <- ggplot(data = birds, aes(x = res_brain, y = clutch_size, col = devo_mode)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()

plot3 <- ggplot(data = birds, aes(x = res_brain, y = egg_mass, col = devo_mode)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()

plot4 <- ggplot(data = birds, aes(x = longevity, y = egg_mass, col = devo_mode)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()

plot5 <- ggplot(data = birds, aes(x = longevity, y = clutch_size, col = devo_mode)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()

ggarrange(plot1, plot2, plot3, nrow = 1, common.legend = T)
ggsave('additional.pdf', device = 'pdf')

ggarrange(plot4, plot5, nrow = 1, common.legend = T)
ggsave('additional2.pdf', device = 'pdf')

plot6 <- ggplot(data = birds, aes(x = longevity, y = res_brain, col = weight)) +
  geom_point() +
  geom_smooth(method = 'lm') + theme_classic()
plot6


#### sensitivity analysis - Feng tree ----
...
tree_feng_data <- birdtree.Feng
# sort(tree_feng_data$edge.length) # check branch lengths
# tree_feng_data$edge.length[tree_jetz_data$edge.length == 0] <- 1e-10
tree_feng_data_AINV <- inverseA(tree_feng_data, nodes = "ALL")

# allocation model - Feng
model_alloc_f_res <- MCMCglmm(res_brain ~ devo_mode + weight + time_fed + res_egg_mass + clutch_size + caretakers +
                                devo_mode:weight + devo_mode:time_fed +
                                res_egg_mass:devo_mode + 
                                res_egg_mass:weight +
                                clutch_size:devo_mode + clutch_size:weight + clutch_size:res_egg_mass,
                              random = ~ tip_label_feng,
                              ginverse = list(tip_label_feng = tree_feng_data_AINV$Ainv),
                              prior = prior1,
                              data = birds2,
                              nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
summary(model_alloc_f_res)
p2 <- model_alloc_f_res$VCV[,"tip_label_feng"]/(model_alloc_f_res$VCV[,"tip_label_feng"]+model_alloc_f_res$VCV[,"units"])
posterior.mode(p2)
HPDinterval(p2)
...
model_combined_f_res <- MCMCglmm(res_brain ~ devo_mode + weight + time_fed + res_egg_mass + clutch_size + caretakers +
                                 ...
                                 ,
                                 random = ~ tip_label_feng,
                                 ginverse = list(tip_label = tree_feng_data_AINV$Ainv),
                                 prior = prior1,
                                 data = birds2,
                                 nitt = 30000, thin = 22, burnin = 8000, verbose = VERB)
summary(model_combined_f_res)
...
birds2$longevity_t <- scale(log(birds$longevity))
